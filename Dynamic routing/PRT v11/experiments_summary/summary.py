...
for i in [1000, 2000, 3000]:
    for j in range(54):
        ...
        with open('ex%d.txt' % num, 'r') as fp:
            ...
            summary_txt.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t\n' % (v_NN, v_meanTimeArrival, v_imbalanceLevel, v_numOfPRTs, v_compuTime, TTDistance, TETDistance , TFTime, TWTime, AWTime, ISTime, ISP, ASTime, ASP, SSTime, SSP, TSTime, TSP, PSTime, PSP)) 
