...
class PRT():
    ...
    def On_IdleToSetting(self, cur_time, target_c):
        waitingFirst_c = target_c.sn.waitCustomerInNode[0]
        if waitingFirst_c != target_c:
            target_c = waitingFirst_c
    ...
    def On_ApproachingToSetting(self, cur_time, target_c):
        print target_c, target_c.sn, self
        waitingFirst_c = target_c.sn.waitCustomerInNode[0]
        if waitingFirst_c != target_c:
            target_c = waitingFirst_c
    ...
    def On_SettingToTransiting(self, cur_time, target_c):
        assert self.transporting_customer == target_c
        logger('%.1f:    On_S2T - %s, setting customer-%s' % (cur_time, self, target_c))
