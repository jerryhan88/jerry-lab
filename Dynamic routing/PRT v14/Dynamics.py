...
class PRT():
    ...
    def re_assign_customer(self, cur_time, target_c):
        if self.state == ST_IDLE :
            if self.arrived_n != target_c.sn:
                # Idle -> Approaching
                x = [cur_time, self.On_IdleToApproaching, target_c.sn]
                self.event_seq.append(x)
                self.On_IdleToApproaching(cur_time, target_c.sn)
            else:
                # self.arrived_n == target_c.sn
                # Idle -> Setting
                x = [cur_time, self.On_IdleToSetting, target_c.sn]
                self.event_seq.append(x)
                self.On_IdleToSetting(cur_time, target_c.sn)
        elif self.state == ST_APPROACHING:
            if self.path_n[-1] != target_c.sn:
                # Approaching -> Approaching
                # Even though this PRT approaching assigned customer
                # the PRT assigned to target_c has been changed
                x = [cur_time, self.On_ApproachingToApproaching, target_c.sn]
                self.event_seq.append(x)
                self.On_ApproachingToApproaching(cur_time, target_c.sn)
            else:
                # There is no assigned customer change
                assert self.path_n[-1] == target_c
    ...
    def On_IdleToSetting(self, cur_time, target_n):
        logger('%.1f:    On_I2S - %s, already arrived station%s(waiting %d)' % (cur_time, self, target_n, len(target_n.waitCustomerInNode)))
        ...
        self.transporting_customer = target_n.waitCustomerInNode.pop(0)
...
#         self.modify_passed_assignedPRT_event(cur_time, prev_PRT, target_c)
    ...
    def On_IdleToApproaching(self, cur_time, target_n):
        logger('%.1f:    On_I2A - %s, going to station%s(waiting %d)' % (cur_time, self, target_n, len(target_n.waitCustomerInNode)))
        assert self.state == ST_IDLE
        ...
        # Set things for next state
        self.path_n, self.path_e = Algorithms.find_SP(self.arrived_n.no, target_n.no)
        self.last_planed_time = cur_time
        evt_change_point = cur_time + sum(e.distance // min(PRT_SPEED, e.maxSpeed) for e in self.path_e)
        x = [evt_change_point, self.On_ApproachingToSetting, target_n]
        self.event_seq.append(x)
        heappush(event_queue, x)
...
#         self.modify_passed_assignedPRT_event(cur_time, prev_PRT, target_c)
    ...
    def On_ApproachingToSetting(self, cur_time, target_n):
        logger('%.1f:    On_A2S - %s, arrived station%s(waiting %d)' % (cur_time, self, target_n, len(target_n.waitCustomerInNode)))
    ...
    def On_SettingToTransiting(self, cur_time, transforting_c):
        assert transforting_c == self.transporting_customer 
        logger('%.1f:    On_S2T - %s, departure with customer-%s' % (cur_time, self, transforting_c))
        ...
        # Set things for next state
        self.path_n, self.path_e = Algorithms.find_SP(transforting_c.sn.no, transforting_c.dn.no)
        self.last_planed_time = cur_time
        evt_change_point = cur_time + sum(e.distance // min(PRT_SPEED, e.maxSpeed) for e in self.path_e)
        x = [evt_change_point, self.On_TransitingToIdle, transforting_c]
        self.event_seq.append(x)
        heappush(event_queue, x)
        
        logger('            path: %s' % (self.path_n))

    def On_ApproachingToApproaching(self, cur_time, target_n):
...
#             self.state = ST_APPROACHING
        logger('%.1f:    On_A2A - %s, change path to station%s' % (cur_time, self, target_n))
        assert self.state == ST_APPROACHING
...
#             A2S_EVT[1] = None        
    ...
    def On_TransitingToIdle(self, cur_time, serviced_c):
        ...
        travel_distance = sum(e.distance for e in self.path_e)
        Total_travel_distance += travel_distance
        Total_customers_flow_time += cur_time - serviced_c.arriving_time
