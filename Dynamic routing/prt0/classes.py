...
class Edge():
    ...
    def draw(self, gc):
        ...
        gc.DrawLines([(ex, ey), (ex - int(ux * 5) - int(px * 3), ey - int(uy * 5) - int(py * 3))])
        gc.SetFont(wx.Font(9, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL))
...
class PRT():
    ...
    def update_pos(self, TIMER_MILSEC, simul_clock, op):
        if self.ETA <= simul_clock:
            ...
            if self.arrived_n == self.dest_n:
                if self.state == 1:
                    ...
                    self.state = 2
                    op.write(' %s sec, PRT%d is departing from N%d with %s\n' % (simul_clock, self.id, self.dest_n.id, self.riding_cus.id));
                else:
                    assert self.state == 2
                    op.write(' %s sec, PRT%d is arrived on N%d with %s\n' % (simul_clock, self.id, self.dest_n.id, self.riding_cus.id));
                    self.riding_cus = None
                    self.state = 0
        else:
