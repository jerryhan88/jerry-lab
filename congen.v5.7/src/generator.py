...
class Generator:
    ...
    def create_containers(self, cargo_dict, Vessel_name):
        ...
        I_container_type = cargo_dict['Import'][0]
        I_number_of_container = cargo_dict['Import'][1]
#        I_POD = cargo_dict['Import'][2]
        for _ in range(I_number_of_container()):
            container_character['Import'].append(Container(I_container_type, '20ft', 'Arrived_XT', None))
        
        E_container_type = cargo_dict['Export'][0]
        E_number_of_container = cargo_dict['Export'][1]
        E_POD = cargo_dict['Export'][2]
        for _ in range(E_number_of_container()):
            container_character['Export'].append(Container(E_container_type,'20ft', Vessel_name, E_POD)) 
        
        
#        print 'cargo_dict[Through] : ',cargo_dict['Through']
        Thr_container_type = cargo_dict['Through'][0]
        Thr_number_of_container = cargo_dict['Through'][1]
        for _ in range(Thr_number_of_container()):
            container_character['Through'].append(Container(Thr_container_type, '20ft'))
                
        for v_name, container_info in cargo_dict['Trans'].items():
            container_type = container_info[0]
            number_of_container = container_info[1]
            POD = container_info[2] 
            for _ in range(number_of_container()):
                container_character['Trans'].append(Container(container_type, '20ft', v_name, POD))
