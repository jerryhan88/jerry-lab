    ...
    def OnTimer(self, evt):
        ...
        for p in self.PRTs:
            ...
            if cN != nN:
                ...
                onLineHalveT = (next_inT - cur_outT) / 2
                n1, n2 = cE._from, cE._to
                fromN, toN = None, None
                if n1 == cN:    fromN, toN = n1, n2
                else:           fromN, toN = n2, n1
                arT = (cur_inT + cur_outT) / 2
                fN = (fromN.px, fromN.py)
                tN = (toN.px, toN.py)
                v1 = calc_vec(fN, tN)
                if self.now < arT + onLineHalveT:
                    l = norm(v1) * (travelT / cE.net_trackTime)
                    p.px, p.py = calc_point(cP, v1, l)
                else:
                    l = norm(v1) * (onLineHalveT / cE.net_trackTime)
                    returnPoint = calc_point(cP, v1, l)
                    travelTAfterRP = self.now - (arT + onLineHalveT)
                    v2 = calc_vec(tN, fN)
                    l = norm(v2) * (travelTAfterRP / cE.net_trackTime)
                    p.px, p.py = calc_point(returnPoint, v2, l)
