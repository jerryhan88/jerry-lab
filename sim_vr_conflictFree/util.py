...
class DragZoomPanel(wx.Panel):
    ...
    def OnLeftDown(self, e):
        self.translate_mode, self.prev_x, self.prev_y = True, e.X, e.Y
        self.SetFocus()
        self.CaptureMouse()
    def OnMotion(self, e):
        if self.translate_mode:
            dx, dy = e.X - self.prev_x, e.Y - self.prev_y
            self.translate_x += dx
            self.translate_y += dy
    ...
    def OnMouseWheel(self, e):
        self.set_scale(self.scale * (self.scale_inc if e.WheelRotation > 0 else (1 / self.scale_inc)), e.X, e.Y)
        self.RefreshGC()
