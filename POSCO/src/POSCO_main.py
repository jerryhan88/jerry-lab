...
class L_frame(wx.Frame):
    def __init__(self, parent, ID, title, pos, size, style=wx.DEFAULT_FRAME_STYLE):
        ...
        p_p = wx.Panel(self.base, -1, pos=(f_size_x * 1 / 6, 100), size=(f_size_x * 2 / 3, 150))
        ...
        self.input_id = wx.TextCtrl(self.base, -1, '200727196', size=(p_p_sx / 5, 40))
        ...
        id_p = wx.Panel(self.base, -1, size=(p_p_sx / 6, 40))
        ...
        pw_p = wx.Panel(self.base, -1, size=(p_p_sx / 6, 40))
        pw_p.SetBackgroundColour(wx.Colour(64, 117, 180, 100))
        ps_p_sx, ps_p_sy = pw_p.GetSize()
        pw_p.SetPosition((id_p_px, id_p_py + id_p_sy + 15))
        ps_p_px, ps_p_py = pw_p.GetPosition()
        pw_p.SetFont(wx.Font(20, wx.FONTFAMILY_ROMAN, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD))
        pw_t = wx.StaticText(pw_p, -1, 'P W')
        pw_x, pw_y = pw_t.GetSize()
        pw_t.SetForegroundColour(wx.Colour(255, 255, 255, 100))
        pw_t.SetPosition(((ps_p_sx - pw_x) / 2 , (ps_p_sy - pw_y) / 2))
        self.input_pw = wx.TextCtrl(self.base, -1, '1234', size=(p_p_sx / 5, 40), style=wx.TE_PASSWORD)
        ...
        co_img = wx.Image('pic/cowork.png', wx.BITMAP_TYPE_PNG).ConvertToBitmap()
        
        w = co_img.GetWidth()
        h = co_img.GetHeight()
        
        wx.StaticBitmap(self.base, -1, co_img, (ps_p_px-w - 70, ps_p_py-30),(w, h))
