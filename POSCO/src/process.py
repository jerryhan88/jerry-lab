...
class Process_info_Viewer(wx.Dialog):
    def __init__(self, selected_process):
        ...
        r_box = wx.StaticBox(self, -1, "", pos=(7, 0), size=(345, 645))
        r_box_px, r_box_py = r_box.GetPosition()
        r_box_sx, r_box_sy = r_box.GetSize()
        ...
        search_p = wx.Panel(self, -1, pos=(repo_p_px, repo_p_py + repo_p_sy), size=(342, 40))
        search_p.SetBackgroundColour(b_color)
        search_input = wx.TextCtrl(search_p, -1, 'Delivery', pos=(10, 3),
        ...
        wx.StaticBitmap(search_p, -1, wx.BitmapFromImage(search_img), pos=(search_input_px + search_input_sx + 3, search_input_py))
        
        partner_finder = wx.ScrolledWindow(self, -1, pos=(repo_p_px, repo_p_py + repo_p_sy + 40),
                                              size=(repo_p_sx - 5, 480))
        partner_finder.SetDoubleBuffered(True)
        partner_finder.SetBackgroundColour(b_color)
        partner_finder.SetScrollRate(1, 1)        
        partner_finder.SetScrollbars(repo_p_sx, 100, 1, 13)
        partner_finder_px, partner_finder_py = partner_finder.GetPosition()
        partner_finder_sx, partner_finder_sy = partner_finder.GetSize()
        
        DHL_img = wx.Image('pic/DHL.png', wx.BITMAP_TYPE_PNG)
        DHL_bit = wx.StaticBitmap(partner_finder, -1, wx.BitmapFromImage(DHL_img), pos=(7, 5))
        DHL_bit_px, DHL_bit_py = DHL_bit.GetPosition()
        DHL_bit_sx, DHL_bit_sy = DHL_bit.GetSize()
        FedEx_img = wx.Image('pic/FedEx.png', wx.BITMAP_TYPE_PNG)
        FedEx_bit = wx.StaticBitmap(partner_finder, -1, wx.BitmapFromImage(FedEx_img), pos=(DHL_bit_px, DHL_bit_py + DHL_bit_sy + 3))
        FedEx_bit_px, FedEx_bit_py = FedEx_bit.GetPosition()
        FedEx_bit_sx, FedEx_bit_sy = FedEx_bit.GetSize()
        EX_img = wx.Image('pic/EX.png', wx.BITMAP_TYPE_PNG)
        wx.StaticBitmap(partner_finder, -1, wx.BitmapFromImage(EX_img), pos=(FedEx_bit_px - 5, FedEx_bit_py + FedEx_bit_sy + 3))
        ...
        info_task_box = wx.StaticBox(self, -1, "", pos=(r_box_px + r_box_sx + 5, r_box_py), size=(535, 280))
        info_task_box_px, info_task_box_py = info_task_box.GetPosition()
        info_task_box_sx, info_task_box_sy = info_task_box.GetSize()     
        info_p = wx.Panel(self, -1, pos=(info_task_box_px + 2, info_task_box_py + 12), size=(190, 50))
        info_p_px, info_p_py = info_p.GetPosition()
        info_p_sx, info_p_sy = info_p.GetSize()
        info_img = wx.Image('pic/information.png', wx.BITMAP_TYPE_PNG)
        wx.StaticBitmap(info_p, -1, wx.BitmapFromImage(info_img.Scale(190, 50)))
        task_p = wx.Panel(self, -1, pos=(info_p_px + info_p_sx, info_p_py), size=(341, 50))
        task_img = wx.Image('pic/task.png', wx.BITMAP_TYPE_PNG)
        wx.StaticBitmap(task_p, -1, wx.BitmapFromImage(task_img.Scale(341, 50)))
    ...
    def display_detail_process(self, choice):
        ...
        detail_process_p = wx.Panel(self, -1, pos=(detail_process_box_px, detail_process_box_py + 5), size=(detail_process_box_sx, detail_process_box_sy - 10))
        detail_process_p_sx, detail_process_p_sy = detail_process_p.GetSize()
#        detail_process_p.SetBackgroundColour(wx.Colour(255,123,21))
        # choice == 0  is detail
        if choice == 0:
            details_t_img = wx.Image('pic/detatils_t.png', wx.BITMAP_TYPE_PNG)
            wx.StaticBitmap(detail_process_p, -1, wx.BitmapFromImage(details_t_img.Scale(detail_process_box_sx - 4, 50)), pos=(1, 3))
            
            self.detail_process_viewer = wx.ScrolledWindow(detail_process_p, -1, pos=(0,60),
                                                  size=(detail_process_box_sx - 5, 245))
...
#            self.detail_process_viewer.SetScrollRate(1,0.1)        
            ...
            last_py += btw_line_size
            wx.StaticText(self.detail_process_viewer, -1, 'Address : - Secret -  ', pos=(sub_px, last_py))
            last_py += btw_line_size
            ...
            bd.SetFont(t_font)
            last_py += btw_line_size
            wx.StaticText(self.detail_process_viewer, -1, '- Secret -', pos=(sub_px, last_py))
            last_py += btw_line_size
            wx.StaticText(self.detail_process_viewer, -1, '------------------------------------------------------------------------------------------', pos=(0, last_py))
            last_py += btw_line_size
            ...
            wx.StaticBitmap(detail_process_p, -1, wx.BitmapFromImage(process_t_img.Scale(detail_process_box_sx - 4, 50)), pos=(1, 3))
            wx.StaticBitmap(detail_process_p, -1, wx.BitmapFromImage(wx.Image('pic/process_ex.png', wx.BITMAP_TYPE_PNG).Scale(detail_process_box_sx - 4, 250)), pos=(1, 50))

        wx.StaticBitmap(detail_process_p, -1, wx.BitmapFromImage(wx.Image('pic/repository_b.png', wx.BITMAP_TYPE_PNG).Scale(detail_process_box_sx - 4, 50)),
                         pos=(1, detail_process_p_sy - 50))
    ...
    def display_info_task(self, choice):
        ...
        if choice == 0:
            ...
            wx.StaticBitmap(self.info_task_viewer_p, -1, wx.BitmapFromImage(partner_img), pos=(50, 25))
...
#            cops_n.SetFont()
            ...
            wx.StaticBitmap(self.info_task_viewer_p, -1, wx.BitmapFromImage(ipad_img.Scale(w / 4.5, h / 4)), pos=(50, 20))
            
            iphone_img = wx.Image('pic/iphone.png', wx.BITMAP_TYPE_PNG)
            w = iphone_img.GetWidth()
            h = iphone_img.GetHeight()
            wx.StaticBitmap(self.info_task_viewer_p, -1, wx.BitmapFromImage(iphone_img.Scale(w / 10, h / 9.4)), pos=(200, 20))
